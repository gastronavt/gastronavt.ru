...
<head>
	...
	<!-- js libs -->
	<script src="<?=NFfunctions::getSiteProtocol()?><?=$_SERVER['HTTP_HOST']?>/assets_new/js/minification/minification.js" ></script>
</head>
<body>
	<?php
	...
	<?php include CORE_FOLDER.'/application/views/telegram/widget/right_menu.php'; //Основное меню ?>
    ...
    <div class="wrapper" style="background:<?=$wrapper_background?>;" > 
		<?php 
			...
			if( App::$send_404 ){
				include CORE_FOLDER.'/application/views/telegram/404_view.php';
			} else{
				if( !$router ){
					return false;
				}
	...
	<?php include CORE_FOLDER.'/application/views/telegram/widget/callback.php'; //Обратная связь ?>
	
	<!-- notification APP -->
	<?php include CORE_FOLDER.'/application/views/telegram/widget/download_app_modal.php'; //Предлагаем утсановить приложение мобильное?>
	...
	<script>
		...
		window.onload = function() {
			...
			$(document).on('click', '.btn-add-product, .btn-mini-add-product', function(e) {
				...
				if(product_count >= 1 ){
					//$('.card_product[data-product-id="'+product_id+'"]').css('border', '5px solid <?=App::$current_organization->main_color_c?>', '!important');
				...
				/*$.get('/telegram/card', function(data){
					...
					'<li class="list-group-item" id="card-product'+product_id+'">'+
						...
						'<div class="row">'+
							...
							'<div class="col-4 px-0 align-self-center" data-toggle="modal" data-target="#openProduct'+product_id+'" class="mb-1 h6 d-block" style="cursor: pointer;">'+
								'<div class="product_name" data-product-id="'+product_id+'" style="color:<?=App::$current_organization->main_color_c?>;">'+product_name+'</div>'+
								'<h5 class="font-weight-normal product_price" data-product-id="'+product_id+'" style="color:<?=App::$current_organization->color_sale_price_c?>;">'+product_price+' ₽</h5>'+
							'</div>'+
...
<?
	...
	if(!empty($yandex) && !empty($yandex['seo_yandex_metrika_c'])) {
?>
	<?=str_replace('^IP^', $_SERVER['REMOTE_ADDR'], NFfunctions::decodeString($yandex['seo_yandex_metrika_c']))?>
<? }?>
<!-- Scripts -->

<?php include CORE_FOLDER.'/application/views/telegram/widget/share_modal.php'; //кнопка Поделиться - модальное окно?>
<?php include CORE_FOLDER.'/application/views/telegram/widget/promocod_modal.php'; //модальное окно с промокодом ?>
<?php include CORE_FOLDER.'/application/views/telegram/widget/error_modal.php'; //модальное окно для вывода ошибок ?>
<?php include CORE_FOLDER.'/application/views/telegram/widget/modal.php'; //рекламные модальные окна?>
<?php if(App::$router->getAction() == 'index') {
	include CORE_FOLDER.'/application/views/telegram/widget/products_modal.php'; //Модальные окна продуктов
} ?>
<?php if(App::$router->getAction() == 'lead') {
	include CORE_FOLDER.'/application/views/telegram/widget/gift_modal.php'; //Модальные окна всплывющего подарка в корзине
} ?>

<?php include CORE_FOLDER.'/application/views/telegram/widget/snow.php'; //падающий снег?>
