...
<div class="bg-white" style="padding-top:50px;padding-left:5px;padding-right:5px;padding-bottom:100px;<?php if(!empty(App::$current_aggregator)){ ?> padding-top:100px; <?php } ?>">
	<div style="max-width:992px; margin:0 auto;margin-top:20px;">
		...
		<div style="margin:0 auto; max-width:500px;">
			<center><img src="<?=NFfunctions::getSiteProtocol()?><?=$_SERVER['HTTP_HOST']?>/assets_new/images/dedmoroz.jpg" style="width:200px;margin-bottom:-14px;"/>
			<p style="max-width:700px; border:2px solid #eee;padding:20px;border-radius:5px;margin:20px;"> 
				Для Вас видео-поздравление от <b>Деда мороза</b>!
				<a href="<?=NFfunctions::getSiteProtocol()?><?=$_SERVER['HTTP_HOST']?>/main/newyear" class="btn btn-lg text-white btn-block btn-rounded shadow" style="background:<?=App::$current_organization->main_color_c?>;padding-top:15px;padding-bottom:15px;margin-top:20px;width:170px;">
					<span>Смотреть 😍 </span>
				</a>
			</p>
			</center>
			...
			if(!empty($preOrderBean->ordrs_orders_id_c)){				
				$order = $db->fetchRow($db->query("
					SELECT 
						...
						oo_cstm.client_money_discount_c,
						oo_cstm.bonuses_accrued_c
				...
				$minus_bonuses = $order['client_money_discount_c'];
				$add_bonuses = $order['bonuses_accrued_c'];
				...
				if($order['pay_method_c'] == '02') {
					$pay_method = 'через приложение Сбербанк-онлайн';
				}
				elseif($order['pay_method_c'] == '04') {
					$pay_method = 'на терминал';
				}
				elseif($order['pay_method_c'] == '01') {
					$pay_method = 'наличными';
				}
				elseif($order['pay_method_c'] == '05') {
					$pay_method = 'QR-код СПБ';
				}	
				...
				$order_name = 'Присваивается номер заказа...';
				
				$minus_bonuses = $_SESSION['old_order']->client_money_discount_c ?? null;
				$add_bonuses = $_SESSION['old_order']->bonuses_accrued_c ?? null;
		...
		?>
			...
			<div class="row m-0 p-3">
				...
				<div class="col-md-6 mt-4 mt-sm-0 ml-0 ml-md-4">
					...
					<? } ?>
					<?php if(!empty($minus_bonuses)) { ?>
						<div class="row align-items-center border-bottom py-3">
							<div class="col-4">
								<img class="rounded" style="max-width:66px;" src="<?=NFfunctions::getSiteProtocol()?><?=$_SERVER['HTTP_HOST']?>/assets_new/images/change_bonus.jpg" />
							</div>
							<div class="col-5">
								<div style="color:<?=App::$current_organization->main_color_c?>;font-size:11pt;">Списано бонусов</div>
							</div>
							<div class="col-3">
								<span style="color:red;">- <?=$minus_bonuses?><i class="material-icons">star_border</i></span>
							</div>
						</div>
					<? } ?>
					<?php if(!empty($add_bonuses)) { ?>
						<div class="row align-items-center border-bottom py-3">
							<div class="col-4">
								<img class="rounded" style="max-width:66px;" src="<?=NFfunctions::getSiteProtocol()?><?=$_SERVER['HTTP_HOST']?>/assets_new/images/change_bonus.jpg" />
							</div>
							<div class="col-5">
								<div style="color:<?=App::$current_organization->main_color_c?>;font-size:11pt;">Бонусов начислим <br><span style="font-size:9pt;">(после доставки заказа)</span><br>Сгорят через <?=$bonuseSystem['bonus_end_days_c']?> дней!</div>
							</div>
							<div class="col-3">
								<span style="color:green;">+ <?=$add_bonuses?><i class="material-icons">star_border</i></span>
							</div>
						</div>
					<? } ?>
					...
					<? } ?>
						<div class="row align-items-center border-bottom py-3">
							<div class="col-9">
								<div style="font-size:16pt;">ИТОГО к оплате</div>
