...
<head>
	<?php
		...
		header("Content-Security-Policy: default-src 'self' 'unsafe-inline' 'unsafe-eval' blob: winmon.ru *.winmon.ru dump.video yandex.net *.yandex.net yastatic.net yandex.ru *.yandex.ru tlgur.com data: *.googleapis.com fonts.gstatic.com vk.com www.w3.org *.vk.com google-analytics.com *.google-analytics.com googletagmanager.com *.googletagmanager.com my.mail.ru *.facebook.net www.facebook.com unpkg.com *.callibri.ru top-fwz1.mail.ru");
	?>
	<!--Мета информация-->
	...
	<?php if(!empty(App::$current_landing->seo_keywords_c)){ ?>
	...
	<link rel="stylesheet" type="text/css" href="<?=NFfunctions::getSiteProtocol()?><?=$_SERVER['HTTP_HOST']?>/assets_new/css/style.php?main_color=<?=App::$current_organization->main_color_c?>&color_scroll=<?=App::$current_organization->color_scroll_c?>&font=<?=App::$current_organization->font_c?>&color_active_btn=<?=App::$current_organization->color_active_btn_c?>&color_category_active_c=<?=App::$current_organization->color_category_active_c?>">
	...
	<script async src="<?=NFfunctions::getSiteProtocol()?><?=$_SERVER['HTTP_HOST']?>/assets_new/js/jquery.maskedinput.min.js"></script>
</head>
<body>
	<?php
	...
	<? 
		if(empty($_SESSION['loader']) || $_SERVER['REQUEST_URI'] == '/webview'){ 
	...
	<?php include CORE_FOLDER.'/application/views/webview/widget/right_menu.php'; //Основное меню ?>
    ...
    <div class="wrapper" style="background:<?=$wrapper_background?>;" > 
		<?php 
			...
			if( App::$send_404 ){
				include CORE_FOLDER.'/application/views/webview/404_view.php';
			} else{
				if( !$router ){
					return false;
				}
	...
	<?php include CORE_FOLDER.'/application/views/webview/widget/callback.php'; //Обратная связь ?>
	...
	<script>
		...
		function animationAddCard(id){
			image = $('img[data-product-id="'+id+'"]');
			image
				.first()
				.clone()
		...
		window.onload = function() {
					...
					'<li class="list-group-item" id="card-product'+product_id+'">'+
						...
						'<div class="row">'+
							'<div class="col-2 pl-0 align-self-center" data-toggle="modal" data-target="#openProduct'+product_id+'" style="cursor: pointer;">'+
								'<img class="product-image h-auto" src="'+product_image+'">'+
							'</div>'+
							'<div class="col-4 px-0 align-self-center" data-toggle="modal" data-target="#openProduct'+product_id+'" class="mb-1 h6 d-block" style="cursor: pointer;">'+
								'<div style="color:<?=App::$current_organization->main_color_c?>;">'+product_name+'</div>'+
								'<h5 class="font-weight-normal" style="color:<?=App::$current_organization->color_sale_price_c?>;">'+product_price+' ₽</h5>'+
							'</div>'+
							'<div class="col-6 align-self-center">'+
								'<div class="input-group input-group-sm" style="margin-left:20px;">'+
									'<div class="input-group-prepend">'+
										...
										'</button>'+
									'</div>'+
									'<input name="count[]" type="text" class="btn product_count" data-product-id="'+product_id+'" style="color:#fff;width:45px;border:none;border-radius:0;background:<?=App::$current_organization->color_product_btn_c?>;" value="'+product_count+'" readonly>'+
			...
			$(document).on('click', '.btn-minus-product', function(e) {
				...
				$('.product_count[data-product-id="'+product_id+'"]').val(product_count);//уменьшаем количество конкретного продукта
...
<?
	$yandex = $db->fetchRow($db->query("
		SELECT 
			yy.*,
			yy_cstm.*	
		FROM yndx_yandex yy
			JOIN yndx_yandex_cstm yy_cstm ON yy_cstm.id_c = yy.id AND yy.deleted = 0
			JOIN lngng_landings ll ON ll.id = yy_cstm.lngng_landings_id_c AND ll.deleted = 0
		WHERE yy_cstm.lngng_landings_id_c = '".App::$current_landing->id."'
	"));

	if(!empty($yandex) && !empty($yandex['seo_yandex_metrika_webview_c'])) {
?>
	<?=str_replace('^IP^', $_SERVER['REMOTE_ADDR'], NFfunctions::decodeString($yandex['seo_yandex_metrika_webview_c']))?>
<? }?>
<!-- Scripts -->

<?php include CORE_FOLDER.'/application/views/webview/widget/share_modal.php'; //кнопка Поделиться - модальное окно?>
<?php include CORE_FOLDER.'/application/views/webview/widget/promocod_modal.php'; //модальное окно с промокодом ?>
<?php include CORE_FOLDER.'/application/views/webview/widget/error_modal.php'; //модальное окно для вывода ошибок ?>

<?php if(App::$router->getAction() == 'index') {
	include CORE_FOLDER.'/application/views/webview/widget/products_modal.php'; //Модальные окна продуктов
} ?>
<?php if(App::$router->getAction() == 'lead') {
	include CORE_FOLDER.'/application/views/webview/widget/gift_modal.php'; //Модальные окна всплывющего подарка в корзине
} ?>

<?php include CORE_FOLDER.'/application/views/webview/widget/modal.php'; //рекламные модальные окна?>

<?php include CORE_FOLDER.'/application/views/webview/widget/snow.php'; //падающий снег?>

<script src="<?=NFfunctions::getSiteProtocol()?><?=$_SERVER['HTTP_HOST']?>/assets_new/js/copyPotection.js" ></script>
</body>
