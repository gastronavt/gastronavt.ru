...
class PostomatController extends Controller
{
	...
	public function index()
	{
		...
		$this->view->generateWithTemplate(
			'main_view.php', 
			'template_main_postomat_view.php',
			$data
		);
	}
	...
	{	
		if(empty($_COOKIE['client_id'])){
			...
			header('HTTP/1.1 200 OK');
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/profile');
			exit();
		}
	}
	...
	{	
		...
		if($this->params[0] == 'login'){
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/login');
		} else {
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST']);
		}
		exit();
	...
	{
		if(!empty($_GET['genitive'])){
			$addname = ' для '.$_GET['genitive'];
		}
		$this->model->getTitle('Поздравление с Новым годом '.$addname.' от '.App::$current_organization->name_rus_c.' #winmon');
		if(isset($_SESSION['current_order'])) {
			$this->model->getOrderProducts($_SESSION['current_order']);
		}
	...
	{	
		...
		if($prdctBean->id){
			$this->model->getTitle('Заказать «‎'.$prdctBean->name.'» в городе '.App::$current_city->name.' | '.App::$current_organization->name_rus_c);
			$this->model->getDescription('«‎'.$prdctBean->name.'» с доставкой на дом | '.str_replace(PHP_EOL, ' ', strip_tags(html_entity_decode($prdctBean->description))));
			if(isset($_SESSION['current_order'])) {
				$this->model->getOrderProducts($_SESSION['current_order']);
			}
	...
	{	
		...
		$queryProducts = $db->query("
			...
			JOIN pdgrp_product_groups_prdct_products_1_c pg_pp ON pg_pp.pdgrp_product_groups_prdct_products_1prdct_products_idb = pp_cstm.id_c AND pg_pp.pdgrp_product_groups_prdct_products_1pdgrp_product_groups_ida = '".$groupProducts['id']."'
	...
	public function duplicate_order(){
		...
		header('HTTP/1.1 200 OK');
		header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/lead');
		exit();
	}
	...
	public function send_job(){
		if (md5($_POST['norobot']) == $_SESSION['job_captcha'])	{ 
			...
			header('HTTP/1.1 200 OK');
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/job?&success=yes');
			exit();
		} else {
			header('HTTP/1.1 200 OK');
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/job?&success=no');
			exit();
		}
	}
	...
	{	
		if($this->params) {
			...
			$news = $db->fetchRow($db->query("
				...
				WHERE (nn.id = '".$this->params[0]."' OR nn_cstm.link_name_c = '".$this->params[0]."') AND nn.deleted = 0
			"));
	...
	public function products_file_yml(){
		...
		$out .= '<url>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'</url>' . "\r\n";// URL главной страницы магазина.
		...
		$queryAllProducts = $db->query("
			SELECT 
				pg_pp.pdgrp_product_groups_prdct_products_1pdgrp_product_groups_ida as group_id, 
				pp.*, 
				pp_cstm.*,
				image.id as product_image_id
			FROM prdct_products pp 
		...
		while($product = $db->fetchByAssoc($queryAllProducts)) {
			...
			$out .= '<url>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/product/'.$product['id'].'</url>'."\r\n";
			...
			//$out .= '<picture>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/upload/'.$product['product_image_id'].'_image_c</picture>'."\r\n";
			$out .= '<picture>http://crm.giveat.ru/index.php?entryPoint=download&id='.$product['product_image_id'].'_image_c&type=img_img_images</picture>'."\r\n";
		...
		$out = str_replace('&', '&amp;', $out); //заменяем символ (например, ролл Киш & Миш)
	...
	//для delivery club
	public function products_file_xml(){
		$out = '<?xml version="1.0" encoding="UTF-8"?>' . "\r\n";
		$out = '<!DOCTYPE dc_catalog SYSTEM "http://www.delivery-club.ru/xml/dc.dtd">' . "\r\n";
		$out .= '<dc_catalog last_update="'.date('Y-m-d').' 00:01">' . "\r\n";
		$out .= '<delivery_service>' . "\r\n";
	...
	public function sitemap(){
		$out = '<?xml version="1.0" encoding="UTF-8"?>';
		$out .= '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd">';
		 
		$out .= '
			<url>
			  <loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'</loc>
			  <lastmod>2022-09-19T19:02:07+01:00</lastmod>
			  <changefreq>daily</changefreq>
			  <priority>1.00</priority>
			</url>
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/area_map</loc>
				<lastmod>2022-09-19T19:02:07+01:00</lastmod>
				<changefreq>weekly</changefreq>
				<priority>0.8</priority>
			</url>
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/contact</loc>
				<lastmod>2022-09-19T19:02:07+01:00</lastmod>
				<changefreq>weekly</changefreq>
				<priority>0.8</priority>
			</url>
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/news</loc>
				<lastmod>2022-09-19T19:02:07+01:00</lastmod>
				<changefreq>daily</changefreq>
				<priority>0.8</priority>
			</url>
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/review</loc>
				<lastmod>2022-09-19T19:02:07+01:00</lastmod>
				<changefreq>daily</changefreq>
				<priority>0.8</priority>
			</url>
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/faq</loc>
				<lastmod>2022-09-19T19:02:07+01:00</lastmod>
				<changefreq>weekly</changefreq>
				<priority>0.8</priority>
			</url>
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/job</loc>
				<lastmod>2022-09-19T19:02:07+01:00</lastmod>
				<changefreq>monthly</changefreq>
				<priority>0.8</priority>
			</url>
			<url>
			  <loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/term_of_use</loc>
			  <lastmod>2017-04-07T09:35:45+00:00</lastmod>
			  <changefreq>monthly</changefreq>
			  <priority>0.5</priority>
			</url>
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/login</loc>
				<lastmod>2022-09-19T19:02:07+01:00</lastmod>
				<changefreq>weekly</changefreq>
				<priority>0.7</priority>
			</url>
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/robots.txt</loc>
				<lastmod>2022-09-19T19:02:08+01:00</lastmod>
				<changefreq>weekly</changefreq>
				<priority>0.7</priority>
			</url>';
		...
		while($product = $db->fetchByAssoc($queryAllProducts)) {
			// URL страницы товара
			$out .= '<url>';
			$out .= '<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/product/'.$product['id'].'</loc>';
			$out .= '<lastmod>2022-09-19T19:02:08+01:00</lastmod>';
		...
		// Список новостей
		global $db;
		$queryAllNews = $db->query("
			SELECT nn.id, nn.date_entered, nn_cstm.link_name_c
			FROM lngng_landings ll
			JOIN lngng_landings_news_news_1_c ll_nn ON ll_nn.lngng_landings_news_news_1lngng_landings_ida = ll.id AND ll.id = '".App::$current_landing->id."' AND ll.deleted = 0 AND ll_nn.deleted = 0
			JOIN news_news nn ON nn.id = ll_nn.lngng_landings_news_news_1news_news_idb AND nn.deleted = 0
			JOIN news_news_cstm nn_cstm ON nn_cstm.id_c = nn.id AND nn.deleted = 0
			ORDER BY nn.date_entered DESC
		");
		while($new = $db->fetchByAssoc($queryAllNews)) {
			$new_link = $new['link_name_c'] ?? $new['id'];

			// URL страницы товара
			$out .= '<url>';
			$out .= '<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/news/'.$new_link.'</loc>';
			$out .= '<lastmod>2022-09-19T19:02:08+01:00</lastmod>';
	...
	public function order_check() {
		if(App::$current_landing->status_c == '02' || in_array($_SESSION['current_order']->pay_method_c, ['01','02','04','05'])){
			header('HTTP/1.1 200 OK');
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/order_offline_pay');
			exit();
			...
			if(!empty(App::$current_landing->sberbank_acquiring_payments_c)){
				...
				$response = $sberbank->register(
					...
					NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/order_online_pay',
					NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/lead',
	...
	public function sberbank_acquiring_success_pay(){
		...
		header('HTTP/1.1 200 OK');
		header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/postomat/order_online_pay');
		exit();
	}
	...
	public function savePreOrder() {
		...
		foreach($_SESSION['current_order']->CUSTOM_products as $productBean){
			...
			$source = $productBean->CUSTOM_source ?? null;
		...
		if(empty($_SESSION['current_order']->client_money_discount_c) && !empty(App::$current_user->id)){
			...
			$bonuseSystem = $db->fetchRow($db->query("
				...
				FROM
					...
					JOIN bnss_bonuses bb ON bb.id = bl.bnss_bonuses_lngng_landings_1bnss_bonuses_ida AND bb.deleted = 0 AND bl.bnss_bonuses_lngng_landings_1lngng_landings_idb = '".App::$current_landing->id."'
	...
	public function cleanCurrentOrder() {
		...
		setcookie('lead_path', '', time() - 3600, '/'); //очищаем куку с источником заказа
	}
	...
	public function body_html(){
		...
		if($_SESSION['current_order']->pay_method_c == '03'){
			...
			$pay_method = 'оплата Сбербанк-онлайн';
		}else if($_SESSION['current_order']->pay_method_c == '01'){
			$pay_method = 'оплата наличными';
		}else if($_SESSION['current_order']->pay_method_c == '04'){
			$pay_method = 'оплата через терминал';
