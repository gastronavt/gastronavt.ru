...
class WebviewController extends Controller
{
	...
	public function index()
	{
		...
		//процесс простановки FIREBASE token
		if(!empty($_GET['token']) && !empty($_GET['type_platform']) && !empty(App::$current_user)){
			$_SESSION['firebase_token'] = $_GET['token'];
			$_SESSION['type_platform'] = $_GET['type_platform'];

			NFfunctions::setFirebaseToken(App::$current_user->phone_c, App::$current_landing->id, $_GET['type_platform'], $_SESSION['firebase_token']);
		}
		...
		$this->view->generateWithTemplate(
			'main_view.php', 
			'template_main_webview_view.php',
			$data
		);
	}
	...
	{	
		if(!empty($_SESSION['current_order']->CUSTOM_products)){
			...
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/webview');
			exit();
		}
	}
	...
	{	
		if(empty($_COOKIE['client_id'])){
			...
			$this->view->generate(
				'webview/login_view.php',
				$this->model->getData()
			);
		} else {
			header('HTTP/1.1 200 OK');
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/webview/profile');
			exit();
		}
	}
	...
	{	
		...
		if($this->params[0] == 'login'){
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/webview/login');
		} else {
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/webview');
		}
		exit();
	...
	public function duplicate_order(){
		...
		header('HTTP/1.1 200 OK');
		header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/webview/lead');
		exit();
	}
	...
	public function send_job(){
		if (md5($_POST['norobot']) == $_SESSION['job_captcha'])	{ 
			...
			header('HTTP/1.1 200 OK');
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/webview/job?&success=yes');
			exit();
		} else {
			header('HTTP/1.1 200 OK');
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/webview/job?&success=no');
			exit();
		}
	}
	...
	{	
		if($this->params) {
			...
			if($this->params[0] == $news['id'] && $news['link_name_c'] != null){
				...
				header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/webview/news/'.$news['link_name_c']);
				exit();
			}
	...
	{	
		...
		if($this->params[0] == $page['id'] && $page['link_name_c'] != null){
			header("HTTP/1.1 301 Moved Permanently");
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/webview/page/'.$page['link_name_c']);
			exit();
		}
	...
	public function products_file_yml(){
		...
		while($product = $db->fetchByAssoc($queryAllProducts)) {
			...
			$out .= '<url>'.App::$current_landing->link_c.'/webview/product/'.$product['id'].'</url>'."\r\n";
	...
	public function order_check() {
		if(App::$current_landing->status_c == '02' || in_array($_SESSION['current_order']->pay_method_c, ['01','02','04','05'])){
			header('HTTP/1.1 200 OK');
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/webview/order_offline_pay');
			exit();
			...
			if(!empty(App::$current_landing->sberbank_acquiring_payments_c)){
				...
				$response = $sberbank->register(
					...
					NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/webview/order_online_pay',
					NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/webview/lead',
	...
	public function sberbank_acquiring_success_pay(){
		...
		header('HTTP/1.1 200 OK');
		header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/webview/order_online_pay');
		exit();
	}
	...
	public function savePreOrder() {
		...
		NFfunctions::setFirebaseToken($preorderBean->phone_c, App::$current_landing->id, $_SESSION['current_order']->lead_path_c , $_SESSION['firebase_token']);
