...
class MainController extends Controller
{
	...
	{	
		if(empty($_COOKIE['client_id'])){
			...
			header('HTTP/1.1 200 OK');
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/profile');
			exit();
		}
	}
	...
	{	
		...
		if($this->params[0] == 'login'){
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/login');
		} else {
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST']);
		}
		exit();
	...
	{
		$addname = !empty($_GET['genitive']) ? ' для '.$_GET['genitive'] : '';
	...
	public function send_job(){
		if (md5($_POST['norobot']) == $_SESSION['job_captcha'])	{ 
			...
			header('HTTP/1.1 200 OK');
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/job?&success=yes');
			exit();
		} else {
			header('HTTP/1.1 200 OK');
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/job?&success=no');
			exit();
		}
	}
	...
	{	
		if($this->params) {
			...
			if($this->params[0] == $news['id'] && $news['link_name_c'] != null){
				...
				header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/news/'.$news['link_name_c']);
				exit();
			}
	...
	{	
		...
		if($this->params[0] == $page['id'] && $page['link_name_c'] != null){
			header("HTTP/1.1 301 Moved Permanently");
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/page/'.$page['link_name_c']);
			exit();
		}
	...
	public function products_file_yml(){
		...
		while($product = $db->fetchByAssoc($queryAllProducts)) {
			...
			$out .= '<url>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/product/'.$product['id'].'</url>'."\r\n";
	...
	public function sitemap(){
		...
		$out .= '
			...
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/area_map</loc>
				<lastmod>2022-09-19T19:02:07+01:00</lastmod>
				<changefreq>weekly</changefreq>
				<priority>0.8</priority>
			</url>
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/contact</loc>
				<lastmod>2022-09-19T19:02:07+01:00</lastmod>
				<changefreq>weekly</changefreq>
				<priority>0.8</priority>
			</url>
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/news</loc>
				<lastmod>2022-09-19T19:02:07+01:00</lastmod>
				<changefreq>daily</changefreq>
				<priority>0.8</priority>
			</url>
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/review</loc>
				<lastmod>2022-09-19T19:02:07+01:00</lastmod>
				<changefreq>daily</changefreq>
				<priority>0.8</priority>
			</url>
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/faq</loc>
				<lastmod>2022-09-19T19:02:07+01:00</lastmod>
				<changefreq>weekly</changefreq>
				<priority>0.8</priority>
			</url>
			<url>
				<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/job</loc>
				<lastmod>2022-09-19T19:02:07+01:00</lastmod>
				<changefreq>monthly</changefreq>
				<priority>0.8</priority>
			</url>
			<url>
			  <loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/term_of_use</loc>
		...
		while($product = $db->fetchByAssoc($queryAllProducts)) {
			// URL страницы товара
			$out .= '<url>';
			$out .= '<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/product/'.$product['id'].'</loc>';
		...
		while($new = $db->fetchByAssoc($queryAllNews)) {
			...
			// URL страницы товара
			$out .= '<url>';
			$out .= '<loc>'.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/news/'.$new_link.'</loc>';
			$out .= '<lastmod>2022-09-19T19:02:08+01:00</lastmod>';
	...
	public function order_check() {
		if(App::$current_landing->status_c == '02' || in_array($_SESSION['current_order']->pay_method_c, ['01','02','04','05'])){
			header('HTTP/1.1 200 OK');
			header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/order_offline_pay');
			exit();
	...
	public function sberbank_acquiring_success_pay(){
		...
		header('HTTP/1.1 200 OK');
		header('Location: '.NFfunctions::getSiteProtocol().$_SERVER['HTTP_HOST'].'/main/order_online_pay');
		exit();
	}
	...
	public function savePreOrder() {
		...
		//рассчитываем бонусы
		$this->updateBonuses();
		$preorderBean->client_money_discount_c = $_SESSION['current_order']->client_money_discount_c ?? null;//сколько списали бонусов
		$preorderBean->bonuses_accrued_c = $_SESSION['current_order']->bonuses_accrued_c ?? null;//сколько начислили бонусов
